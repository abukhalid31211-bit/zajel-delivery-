...
export default function Register() {
  ...
  const next = () => {
    if (step === 1) {
      if (!form.name || !form.phone || !form.gmail || !form.gov || !form.pass) return setError('يرجى ملء جميع الحقول المطلوبة')
      if (!form.gmail.includes('@')) return setError('أدخل بريد Gmail صحيح')
    ...
    if (step === 2 && !uploaded.every(Boolean)) return setError('يرجى رفع جميع الوثائق الأربع')
    setError('')
    setStep((s) => s + 1)
  }
              ...
              <div className="grid grid-cols-2 gap-2">
                {vehicles.map((v, i) => (
                  <button
                    ...
                    className={`flex items-center gap-2 rounded-2xl border px-3.5 py-3 text-xs font-semibold transition-all ${vehicle === i ? 'border-black bg-black text-white' : 'border-line bg-white text-mute'}`}
                  >
              ...
              {form.pass && (
                <div className="mt-2 flex items-center gap-2">
                  <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-line">
                    <div className="h-full bg-black transition-all" style={{ width: form.pass.length < 6 ? '33%' : form.pass.length <= 8 ? '66%' : '100%' }} />
                  </div>
                  <span className="text-[10px] font-medium text-mute">{form.pass.length < 6 ? 'ضعيفة' : form.pass.length <= 8 ? 'متوسطة' : 'قوية'}</span>
                </div>
              )}
            </div>
            <div>
          ...
          <>
            ...
            {docs.map((d, i) => (
              <button
                ...
                className={`card flex w-full items-center gap-4 p-4 text-right transition-all ${uploaded[i] ? 'border-black' : 'border-dashed'}`}
              >
                ...
                <div className="flex-1">
                  ...
                  <p className={`mt-0.5 text-[10px] font-medium ${uploaded[i] ? 'text-black' : 'text-faint'}`}>
                    {uploaded[i] ? '✓ تم الرفع — اضغط للحذف وإعادة الرفع' : 'لم يتم الرفع — اضغط للتقاط صورة أو الاختيار من المعرض'}
                  </p>
                </div>
                {uploaded[i] && <X className="h-4 w-4 text-faint" />}
              </button>
            ))}
          </>
        )}
