...
export default function TabLayout() {
  return (
    <div className="app-shell">
      ...
      <nav
        className="fixed bottom-0 z-30 mx-auto w-full max-w-[430px] border-t border-line bg-white/95 backdrop-blur"
        ...
        <div className="flex items-stretch justify-around px-2 pb-[max(env(safe-area-inset-bottom),10px)] pt-2">
          {tabs.map((t) => (
            <NavLink
              ...
              to={t.to}
              className={({ isActive }) =>
                `flex flex-1 flex-col items-center gap-1 rounded-2xl py-1.5 transition-colors ${isActive ? 'text-gold-strong' : 'text-faint'}`
              }
            >
              {({ isActive }) => (
                <>
                  <span
                    className={`flex h-9 w-14 items-center justify-center rounded-full transition-colors ${
                      isActive ? 'bg-gold text-white shadow-md shadow-gold/30' : ''
                    }`}
                  >
